...
int tFunc(char* fname){             // Find the file in current directory and print the contents of the file
    char ch;
    FILE* fp = fopen(fname, "r");
    if(!fp){                        // if the file is not exist in current directory
        printf("Error. No such file exists.\n");
        return 0;
    }
    while((ch = fgetc(fp))!=EOF)    // read the contents of the file until the end of file
        putchar(ch);
    fclose(fp);

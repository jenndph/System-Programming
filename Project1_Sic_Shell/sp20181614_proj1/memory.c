...
int duFunc(char* s, char* e){                   // Print the contents of the memory of the range
    ...
    if(s==NULL && e==NULL){                     // dump
        if(addrSave!=0) addrSave += 0x01;       // case that dump starts with 0x00000 address
        start = addrSave;                       // start address is (last printed address + 1)
...
int eFunc(char* a, char* v){                            // Change the value of the address to new one
    ...
    row = addr/16; col = addr%16;                       // get a index of dumpMem[][] using address
    dumpMem[row][col] = value;                          // change the value of the address
...
int fFunc(char* s, char* e, char* v){                   // Change the value from start to end to new one
    ...
    for(int tmp=start; tmp<=end; ){
        for(int i=0; i<16; i++){                        // i means the column of dumpMem[][]
            if(row*16+i>=start && row*16+i<=end){       // if index address is in the range
                dumpMem[row][i] = value;                // change the value to a new one
                tmp += 0x01;                            // increase address pointer value
            }
        }
        row += 1;
    }
...
void resetFunc(){       // Change all value in the memory to 0x00 (initialization)
    int i, j;
    for(i=0; i<65536; i++)
        for(j=0; j<16; j++)
            dumpMem[i][j] = 0x00;
